...
```cmd
C:\Users\114514>go env GOROOT
C:\Program Files\Go

C:\Users\114514>go env GOPATH
C:\Users\114514\go
```
...
![image-20241004222117434](D:\work_save\typora_picture\image-20241004222117434.png)
...
- 切片是一个 **长度可变的数组**，长度可以在运行时修改；
- 切片提供了计算容量的函数 `cap()` 可以测量切片最长可以达到多少：它等于切片的长度 + 数组除切片之外的长度。切片的长度永远不会超过它的容量，所以对于切片 `s` 来说该不等式永远成立：`0 <= len(s) <= cap(s)`。
- 一个切片 `s` 可以这样扩展到它的大小上限：`s = s[:cap(s)]`，如果再扩大的话就会导致运行时错误（参见第 7.7 节）。

> **注意** 绝对不要用指针指向切片。切片本身已经是一个引用类型，所以***它本身就是一个指针***！！
>
...
当使用 `var test []byte` 声明一个切片时，创建了一个空的、未初始化的切片。这种情况下，切片的行为有一些特殊之处：

1. 长度和容量： 长度（len(test)）为 0，容量（cap(test)）也为 0
...
3. 添加元素： 如果你尝试向这个切片添加元素（例如使用 append），Go 会自动分配一个新的底层数组：
...
当使用`slice1 := make([]type, len)`声明一个切片时，切片`[0, len-1]`的值都是可访问的零值，`append`从`len`开始添加。
...
一般可以将不同功能的函数放在不同的目录下，多个`main`函数，例如服务端和客户端，分别放在：`./cmd/server`和`./cmd/client`下。

编译：

```cmd
go build -o serverApp.exe ./cmd/server
go build -o clientApp.exe ./cmd/client
```
...
- 在非`main()`函数中执行协程，函数本身继续执行，函数本身退出不影响协程函数的继续执行。
- 在 `main()`函数本身或以外执行的协程，会随`main()`本身的执行结束而结束，这是因为`main()`的结束代表了整个进程的终止。
- 协程开启本身，会消耗一点时间

代码：

```go
func sleep(str string, num int) {
	for i := 0; i < num; i++ {
		fmt.Printf("%s: %d\n", str, i)
